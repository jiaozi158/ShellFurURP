...
float4 frag(g2f input) : SV_Target
{
    ...
    float AO = (1.0-SAMPLE_TEXTURE2D(_AOMap, sampler_AOMap, input.uv / _BaseMap_ST.xy).x)* _Occlusion;
...
#endif
    {
        float3 mainLightColor = mainLight.color.rgb * mainLight.distanceAttenuation;
        // "Screen" blend mode.
        ...
        //color.rgb += mainLightColor * FurDiffuseKajiya(s, GetMainLight().direction, viewDirWS, normalWS, 0.0, _Backlit, _Area);
    }

    // Calculate all additional lights, you can disable it if needed.
    ///*
...
#endif
        {
            float3 lightColor = light.color.rgb * light.distanceAttenuation;
            // "Screen" blend mode.
    ...
    color.rgb = -min(-color.rgb, float3(0.0, 0.0, 0.0));
#endif
